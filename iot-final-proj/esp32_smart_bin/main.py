...
def setup_sensors():
    ...
    try:
        i2c = machine.I2C(
            0, sda=machine.Pin(LCD_SDA_PIN), scl=machine.Pin(LCD_SCL_PIN), freq=400000
        )
...
def main():
    ...
    while True:
        ...
        if (
            time.ticks_diff(current_time, last_sensor_read_time)
        ...
        if (
            lid_open
            and time.ticks_diff(current_time, lid_open_time) >= LID_OPEN_DURATION_MS
        ):
            close_lid()
